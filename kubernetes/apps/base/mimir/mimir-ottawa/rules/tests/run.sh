...
for rule in flux bhaiya bhaiya-model garage node-clock velero-integrity bhaiya-workspace-image bhaiya-workspace-endpoints; do
  case "$rule" in
    flux) test_file=flux_test.yaml ;;
    bhaiya) test_file=bhaiya_test.yaml ;;
