...
variable "coder_agent_url_override" {
  default     = ""
  description = "Override the Coder agent URL for internal cluster access (e.g., http://coder.coder.svc.cluster.local)"
  type        = string
}
...
locals {
  ...
  # Auto-detect if we should use internal cluster URL
  needs_internal_url = can(regex("coder\\.rajsingh\\.info", data.coder_workspace.me.access_url))
  coder_agent_url = var.coder_agent_url_override != "" ? var.coder_agent_url_override : (
    local.needs_internal_url ? "http://coder.coder.svc.cluster.local" : replace(data.coder_workspace.me.access_url, "/localhost|127\\.0\\.0\\.1/", "host.docker.internal")
  )
  envbuilder_env = {
    "CODER_AGENT_TOKEN" : coder_agent.main.token,
    # Use internal cluster URL if accessing via Tailscale, otherwise use access URL
...
# Commented out to avoid interactive prompt
# module "jetbrains" {
#   count      = data.coder_workspace.me.start_count
